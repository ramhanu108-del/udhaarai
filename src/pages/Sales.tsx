...
import { getSales, getSalesSummary, getTodaySalesSummary } from '../store/selectors';
import { formatCurrency } from '../utils';
import { startOfDay, endOfDay, startOfWeek, endOfWeek, startOfMonth, endOfMonth, format } from 'date-fns';
import { useNavigate } from 'react-router-dom';
import { ArrowLeft, CheckCircle, TrendingUp, NotebookText, XCircle, MoreVertical, X, Settings } from 'lucide-react';
import { PaymentMode, Sale } from '../types';

export const Sales = () => {
  const navigate = useNavigate();
  const { customers, voidSale, sales } = useStore();
  ...
  const periodActiveSales = allSales.filter(s => s.createdAt >= start && s.createdAt <= end && s.status === 'active');
    ...
    <div className="flex flex-col h-full bg-slate-50">
          ...
          {['today', 'week', 'month'].map(period => (
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider shrink-0 transition-colors ${
                filterPeriod === period ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-600'
              }`}
            >
              {period}
            </button>
      ...
      <div className="flex-1 overflow-y-auto w-full px-6 py-4">
         ...
         <div className="bg-emerald-600 rounded-2xl p-5 text-white shadow-lg relative overflow-hidden mb-5">
           ...
           <p className="text-emerald-100 text-[10px] font-bold uppercase tracking-widest mb-1">
             {filterPeriod === 'today' ? 'Aaj ki Sales' : filterPeriod === 'week' ? 'Week ki Sales' : 'Month ki Sales'}
           </p>
           <h3 className="text-3xl font-bold mb-4">{formatCurrency(summary.totalSalesPaise)}</h3>
           <div className="grid grid-cols-4 gap-2 relative z-10 w-full mb-1">
              <div>
                <p className="text-[9px] text-emerald-200 uppercase font-bold tracking-wider">Cash Aaya</p>
                <p className="text-xs font-bold">{formatCurrency(summary.cashPaise)}</p>
              </div>
               <div>
                <p className="text-[9px] text-emerald-200 uppercase font-bold tracking-wider">UPI Aaya</p>
                <p className="text-xs font-bold">{formatCurrency(summary.upiPaise)}</p>
              </div>
               <div>
                <p className="text-[9px] text-emerald-200 uppercase font-bold tracking-wider">Card</p>
                <p className="text-xs font-bold">{formatCurrency(summary.cardPaise)}</p>
              </div>
               <div>
                <p className="text-[9px] text-yellow-300 uppercase font-bold tracking-wider">Udhaar Sale</p>
                <p className="text-xs font-bold text-yellow-300">{formatCurrency(summary.udhaarPaise)}</p>
              </div>
           </div>
           {summary.profitPaise > 0 && (
             <div className="mt-3 pt-3 border-t border-emerald-500/50 flex justify-between items-center">
               <span className="text-[10px] uppercase font-bold tracking-wider text-emerald-100">Profit Estimate</span>
               <span className="text-sm font-bold bg-white/20 px-2 py-0.5 rounded">{formatCurrency(summary.profitPaise)}</span>
             </div>
           )}
         </div>
            ...
            {['all', 'cash', 'upi', 'card', 'udhaar'].map(mode => (
              <button
                ...
                className={`px-3 py-1 rounded text-[10px] font-bold uppercase tracking-wider transition-colors ${
                  filterMode === mode ? 'bg-slate-800 text-white' : 'bg-slate-200 text-slate-600'
                }`}
              >
                {mode}
              </button>
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        <p className={`font-bold text-sm ${sale.status === 'void' ? 'text-red-900 line-through' : 'text-slate-900'}`}>
                          {mainItem?.name || 'Items'} 
                          {sale.items.length > 1 && <span className="text-[10px] text-slate-500 ml-1">+{sale.items.length - 1} more</span>}
                        </p>
                        <div className="flex gap-2 items-center mt-1">
                          {sale.status === 'void' ? (
                            <span className="text-[8px] uppercase font-bold tracking-wider px-1.5 py-0.5 rounded bg-red-100 text-red-700">VOIDED</span>
                          ) : (
                            <span className={`text-[8px] uppercase font-bold tracking-wider px-1.5 py-0.5 rounded ${
                              isUdhaar ? 'bg-red-100 text-red-600' : 'bg-indigo-100 text-indigo-600'
                            }`}>
                              {sale.paymentMode}
                            </span>
                          )}
                          <span className="text-[10px] text-slate-500">{format(sale.createdAt, "h:mm a")}</span>
                        </div>
                        {customer && (
                        ...
                        <div className="flex flex-wrap gap-2 mt-1.5">
                           {mainItem?.inventoryItemId ? (
                             <>
                               <span className="text-[9px] font-bold bg-indigo-50 text-indigo-600 px-1.5 py-0.5 rounded border border-indigo-100">Inventory Linked</span>
                               {mainItem.stockReducedQty ? (
                                  <span className="text-[9px] font-bold bg-emerald-50 text-emerald-600 px-1.5 py-0.5 rounded border border-emerald-100">Stock Reduced: {mainItem.stockReducedQty}</span>
                               ) : null}
                             </>
                           ) : (
                               <span className="text-[9px] font-bold bg-amber-50 text-amber-600 px-1.5 py-0.5 rounded border border-amber-100">Manual Sale</span>
                           )}
                        </div>
                      </div>

                      <div className="text-right flex flex-col items-end">
                        <p className={`font-bold ${sale.status === 'void' ? 'text-slate-400 line-through' : 'text-slate-900'}`}>{formatCurrency(sale.totalPaise)}</p>
                        <button 
                          type="button"
                          onClick={(e) => {
                            e.preventDefault();
      ...
      {selectedSale && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 sm:items-center" onClick={() => { setSelectedSale(null); setVoidConfirmMode(false); }}>
          <div className="w-full bg-white rounded-t-2xl sm:rounded-xl p-6 sm:max-w-sm sm:w-[400px] animate-in slide-in-from-bottom" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <h3 className="font-bold text-slate-900 text-lg">Sale Action</h3>
              <button 
                onClick={() => { setSelectedSale(null); setVoidConfirmMode(false); }} 
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center bg-slate-50 p-3 rounded-lg border border-slate-100">
                ...
                <span className="font-bold text-slate-900">{formatCurrency(selectedSale.totalPaise)}</span>
              </div>
              ...
              {!voidConfirmMode ? (
                <>
                  {selectedSale.status !== 'void' && (
                    <button
                      type="button"
                      onClick={() => navigate('/invoices/new', { state: { fromSale: selectedSale } })}
                ...
                <div className="bg-red-50 p-4 rounded-xl border border-red-100 space-y-4">
                  ...
                  <p className="text-sm text-red-700">Ye sale cancel ho jayegi. Sales total se remove hogi. Agar inventory linked hai to stock restore hoga.</p>
