...
export const Udhaar = () => {
  ...
  const highPendingCustomers = customers.filter(
    (c) => getCustomerBalance(c.id) > 100000,
  ); // 1000 Rs
  ...
  const remainingExpected = overdueCustomers.reduce(
        ...
        <div className="flex flex-col gap-3 mt-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-emerald-50 rounded-xl p-3 border border-emerald-100 flex flex-col justify-center">
              <p className="text-[9px] uppercase font-bold text-emerald-600 mb-1">
                Aaj Udhaar Collection
              </p>
              <p className="text-lg font-bold text-emerald-900">
                {formatCurrency(collectedToday)}
              </p>
              <p className="text-[8px] text-emerald-700 mt-0.5 opacity-80 leading-tight">
                Sirf customer payments count hoti hain, sales nahi.
              </p>
            </div>
            <div className="bg-indigo-50 rounded-xl p-3 border border-indigo-100 flex flex-col justify-center">
              ...
              <p className="text-[8px] text-indigo-700 mt-0.5 opacity-80 leading-tight">
                Sab customers ka total baki paisa.
              </p>
            </div>
          </div>
          <div className="bg-red-50 rounded-xl p-3 border border-red-100">
            ...
            <p className="text-lg font-bold text-red-900">
              {formatCurrency(remainingExpected)}
            </p>
            <p className="text-[8px] text-red-700 mt-0.5 opacity-80 leading-tight">
              Due date cross hone wala amount.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 px-6 py-4 pb-24">
        ...
        {collectionList.length === 0 ? (
          <div className="text-center py-6 text-slate-400 flex flex-col items-center">
            ...
            <p className="text-sm">No overdue or high-risk udhaar.</p>
          </div>
        ) : (
                  ...
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <div className="text-right">
                      ...
                      {overdueIds.includes(customer.id) ? (
                        <span className="text-[8px] uppercase font-bold tracking-wider bg-red-100 text-red-600 px-1 py-0.5 rounded">
                          Overdue
                        </span>
                      ) : (
                        <span className="text-[8px] uppercase font-bold tracking-wider bg-orange-100 text-orange-600 px-1 py-0.5 rounded">
                          High Bal
                        </span>
                      )}
                    </div>
        ...
        <div className="mt-8">
          <div className="mb-4">
            ...
            <p className="text-[10px] text-slate-400 mt-0.5">
              Jinke paas abhi bhi paisa pending hai.
            </p>
          </div>

          {allPendingCustomers.length === 0 ? (
            <div className="text-center py-8 text-slate-400 flex flex-col items-center bg-white rounded-xl border border-slate-200 border-dashed">
              <FileText className="w-8 h-8 mb-2 stroke-1 text-slate-300" />
              <p className="text-sm">Sab customers settled hain.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {allPendingCustomers.map((customer) => (
                <div
                  key={`all-${customer.id}`}
                  className="bg-white rounded-xl p-3 border border-slate-200"
                >
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 bg-indigo-50 rounded-full flex items-center justify-center text-indigo-500 font-bold text-xs border border-indigo-100">
                        {customer.name.charAt(0).toUpperCase()}
                      </div>
                      <div
                        onClick={() => navigate(`/customers/${customer.id}`)}
                    ...
                    <div className="text-right">
                      <p className="font-bold text-red-600 text-sm">
                        {formatCurrency(customer.calculatedBalance)}
                      </p>
                      <span className="text-[8px] uppercase font-bold tracking-wider bg-slate-100 text-slate-600 px-1 py-0.5 rounded">
                        Due
                      </span>
                    </div>
                  </div>
